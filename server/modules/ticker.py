...
class TickerModule(BaseModule):
    ...
    config_schema = [
        ...
        {"key": "message_interval_seconds", "label": "文字輪替秒數（0＝固定第一句）", "type": "number", "default": 120, "min": 0, "max": 86400},
        {"key": "scroll_step_seconds", "label": "捲動更新秒數", "type": "number", "default": 10, "min": 5, "max": 3600, "help": "每次變動一次才局刷；數字越小，跑得越順也越常局刷。"},
    ...
    def render(self, data, size, color_mode, cfg):
        ...
        try:
            message_interval = max(0, min(86400, int(cfg.get("message_interval_seconds", 120))))
            step_seconds = max(5, min(3600, int(cfg.get("scroll_step_seconds", 10))))
